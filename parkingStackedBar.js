...
module.exports = {
    parkingStackedBar: function(callback){
        let height = 700;
        let width = 900;
        ...
        $.getJSON('http://localhost:8080/citationData.json', datafile=>{
            ...
            let colorDomain = _.uniq(_.map(datafile, 'series'));
            let byMonthForChart = _.keyBy(xDomain);
            
            // scales
            let xScale = d3.scaleBand().range([0, width]).domain(xDomain).padding(0.1);
            ...
            d3.select('#svg0').append('text')
                ...
                .attr("y", margin.top/2)
                .text('Baltimore City Parking Citations by Month Added to DB')
